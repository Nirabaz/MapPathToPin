...
class APIManager {
    ...
    private func fireRequestWith(router: BaseRouter, completionHandler: @escaping api_success_block) {
        if isOnline(){
            ...
            Utils.showAlertWithMessage(message: "There is no internet connection", title: "Error")
        }
    }
    
    func getOrderByCoordinates(lat: Double, lon: Double, success: @escaping api_success_block) {
        let router = OrderRouter(anEndpoint: .GetOrderByCoordinates(lat,  lon, nil))
